...
def run_training(data_type="screw",
    ...
    optimizer = torch.optim.Adam([
                	{'params': blur.parameters(), 'lr':learninig_rate}, 
                	{'params': SRNet.parameters(), 'lr':learninig_rate}, 
   	            ])
    ...
    for step in tqdm(range(epochs)):
        ...
        oimg = data[0].cuda()#原图 (16,3,300,300)
        timg = data[1].cuda()#转换图(16,3,300,300)
        ...
        loss1 = mse(oimg, sr_oimg)
        #loss2:复原缺陷图和原图尽可能相似，优化超分网络，帮助超分后的图像掩盖缺陷
        loss2 = mse(oimg, sr_timg)
        #loss3:复原缺陷图和缺陷图之间存在对比损失，
        #loss3= 
        #loss3：模糊后的原图和模糊后的缺陷图之间尽可能相似，帮助模糊缺陷
        loss3 = mse(blur_oimg, blur_timg) 
        ...
        print("loss1:",round(loss1.item(),2),"loss2:",round(loss2.item(),2),"loss3:",round(loss3.item(),2))
        print("epoch:",step,"      loss:   ",round(loss.item(),2)) 
        ...
        if test_epochs > 0 and epoch % test_epochs == 0:
            ...
            with torch.no_grad():
                ...
                roc_auc = auc(fpr, tpr)
                print(epoch,step,"eval_auc:", round(roc_auc,2))

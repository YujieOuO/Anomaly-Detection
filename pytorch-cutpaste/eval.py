...
def eval_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8):
    ...
    if model is None:
        ...
        load_models = torch.load(modelname)
        blur.load_state_dict(load_models['blur'])
        SRNet.load_state_dict(load_models['SRNet'])
    ...
    with torch.no_grad():
        for x, label in dataloader_test:
            ...
            score = []
            for i in range(64):
                score = - torch.nn.MSELoss(oimg[i],srimg[i])
            #print(embed.shape) (64,512)
            #print(logit.shape) 
    ...
    # claculate mean
    #将该类所有的训练数据，在第一维度取平均得到[512]，每个通道变成了所有数据在该位置的平均值
    '''
